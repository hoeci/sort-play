(async function () {
  ...
  function showCredentialsModal() {
    ...
    Spicetify.PopupModal.display({
      title: "<span style='font-size: 25px;'>Spotify API Credentials</span>", // Change font-size here
    ...
    saveButton.addEventListener("click", async () => {
      ...
      saveButton.style.backgroundColor = "#B3B3B3"; // Gray out when disabled
    ...
    const confirmButton = Spicetify.React.createElement(
      "button",
      {
        ...
        style: {
          ...
          border: "none",
          cursor: "pointer",
          backgroundColor: "#FF4444", // Red color for delete
          color: "white",
          fontWeight: "550",
          fontSize: "13px",
  ...
  // Function to get access token (client credentials flow)
  ...
  // Function to get track details along with play count (with retries)
  ...
  function animateButtonText(button) {
    ...
    }, 150); // Adjust the interval (in milliseconds) for faster/slower animation
  ...
  button.addEventListener("click", () => {
    button.style.backgroundColor = "#B3B3B3"; // Light gray
    button.style.color = "#666"; // Darker text
    button.style.cursor = "default"; // Disable hover effects
    button.disabled = true; // Disable to prevent further clicks
  });
  ...
  async function processBatchesWithDelay(tracks, batchSize = 200, delayMs = 1000) {
    const results = [];
    const batches = [];
    let processedTrackCount = 0; // Keep track of processed tracks
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      batchResults.forEach((result) => {
        if (result.status === "fulfilled" && result.value !== null) {
          results.push(result.value); // Add successful result
          processedTrackCount++;
        } else {
  ...
  async function addTracksToPlaylist(playlistId, trackUris) {
    ...
    // 1. Filter out invalid URIs and remove duplicates
    ...
    for (let i = 0; i < validAndUniqueUris.length; i += BATCH_SIZE) {
      ...
      try {
        ...
        // Consider retrying the batch or adding tracks one by one if a batch fails
      }
    }
  }
  ...
  button.addEventListener("click", async (event) => {
    ...
    const creds = loadCredentials();
    if (!creds.clientId || !creds.clientSecret) {
      button.disabled = true; // Disable button when showing modal
      button.style.backgroundColor = "#B3B3B3"; // Light gray
      button.style.color = "#666"; // Darker text
      button.style.cursor = "default"; // Disable hover effects
      showCredentialsModal();
    ...
    // Start the animation *before* fetching data
    ...
    try {
      ...
      if (playlistId) {
        ...
        // Filter out tracks with "N/A" play counts and sort by play count (descending)
        ...
        sortedTracks.forEach((track, index) => {
          ...
          const durationSeconds = ((track.durationMs % 60000) / 1000).toFixed(
            0
          );
          const formattedDuration = `${durationMinutes}:${durationSeconds.padStart(
            2,
            "0"
          )}`;
          console.log(
    ...
    if (insertionPoint) {
      try {
        ...
        if (insertionPoint.firstChild) {
          ...
          // Otherwise, just append (container might be empty)
